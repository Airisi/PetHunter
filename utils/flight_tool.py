...
class FlightMeasureTool(QObject):
    """Flight measurement helper with fixed-interval scheduling (no drift)."""
    ...
    def start_measure(self, fly_times):
        ...
        self._schedule_start_time = time.perf_counter()
    ...
    def _trigger_fly(self):
        ...
        self._fly_start_time = (
            self._schedule_start_time +
            (self.fly_count_done - 1) * self.fly_duration
        )

        self.state_changed.emit(
            f"第 {self.fly_count_done}/{self.fly_count_total} 次上升中"
        )
    ...
    def _update_countdown(self):
        ...
        elapsed = now - self._fly_start_time
        remaining = max(0, int(self.fly_duration * 1000 - elapsed * 1000))
        self.time_remaining_changed.emit(remaining)

        progress = (
            1.0 if self.fly_duration <= 0
            else min(1.0, elapsed / self.fly_duration)
        )

        base_height = max(0, self.fly_count_done - 1) * self.SINGLE_FLY_HEIGHT
        preview_height = round(
            base_height + self.SINGLE_FLY_HEIGHT * progress, 2
        )
    ...
    def _end_fly(self):
        ...
        now = time.perf_counter()
        actual_duration = now - self._fly_start_time
        ...
        self.fly_finished.emit(
            self.fly_count_done,
            self.fly_count_total
        )

        rise_speed = (
            self.SINGLE_FLY_HEIGHT / actual_duration
            if actual_duration > 0 else 0.0
        )

        self.state_changed.emit(
            f"本次上升完成，速度 {rise_speed:.2f} m/s"
        )
    ...
    def mark_land(self, land_height=0.0):
        ...
        if fall_height <= 0:
            ...
            fall_speed = (
                fall_height / fall_time if fall_time > 0 else 0.0
            )

        result = {
            "single_height": round(self.SINGLE_FLY_HEIGHT, 2),
            "fly_times": self.fly_count_total,
            "total_fly_time": round(
                self.fly_duration * self.fly_count_total, 3
            ),
            "total_rise": round(total_rise, 2),
            "rise_speed": round(
                self.SINGLE_FLY_HEIGHT / self.fly_duration, 2
            ) if self.fly_duration > 0 else 0.0,
